...
$$H0 : \beta_{j} = 0 \text{   vs   }HA :\beta_{j} \neq 0$$

The distribution of the test statistic is no longer T since the t-test for ordinary linear regression, assuming the assumptions were correct, had an exact distribution for any sample size.
