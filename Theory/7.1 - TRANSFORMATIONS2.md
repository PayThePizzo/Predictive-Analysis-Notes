...
where $x_{i} = t_{i}$ which means $\mathbb{E}[Y_{i}|t_{i}] = \beta_{0} + \beta_{1}t_{i}$
...
Intuitively we can pass to the supposition that $\beta_{1}$ is changing with respect to time i.e $\beta_{i}(t)$. We consider $\beta_{1}$ a function of the time.
...
A model with a quadratic term corresponds to a model in which the slope is allowed to change as a function of the predictor. In linear model the first derivative is constant, with quadratic terms the first derivative varies. Similar concepts apply to higher order polynomials.
