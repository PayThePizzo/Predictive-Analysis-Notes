...
```
Altrimenti possiamo derivare degli intervalli di confidenza con

```{r}
confint(fit1, parm="age", level = 0.98)

# or

cbind(
    coefficients(fit1)[3] + (qt(.0+alpha/2, df=fit1$df.residual) * sqrt(vcov(fit1)[3,3])),
    coefficients(fit1)[3] + (qt((1-alpha/2),df=fit1$df.residual) * sqrt(vcov(fit1)[3,3]))
)
```
...
conf <- predict(fit2, newdata=nd, interval="confidence")
pred <- predict(fit2, newdata=nd, interval="prediction")

c("Confidence", conf[1,3]-conf[1,2], conf[2,3]-conf[2,2])
c("Prediction", pred[1,3]-pred[1,2], pred[2,3]-pred[2,2])
```
