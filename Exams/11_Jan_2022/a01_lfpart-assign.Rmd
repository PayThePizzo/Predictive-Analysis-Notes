...
summary(fit1)
```
Nel `summary` del modello troviamo un valore particolarmente alto dell' $R^{2}$ e della F-statistic, per un p-value molto piccolo. Percio', il modello sembra essere particolarmente significativo.
...
lines(sort(dex2$numAccess), 
      predict(fit_numAccess, newdata = data.frame(numAccess = sort(dex2$numAccess)), type = "response"), 
      col = "red")
...
rpred <- predict(fitTot, type = "response", newdata = nd, level=0.95)
```
