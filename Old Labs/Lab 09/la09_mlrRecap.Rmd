...
* BFI: Baseflo index - un indice della permeabilità del bacino 
...
* Ireland: in indicatore che indica se il bacino si trova in Irlanda o no (i.e. in Gran Bretagna). variabile non da considerarsi predittiva. 
...
La relazione tra Area e `lqmed` sembra essere non-linear - usiamo quindi la trasformazione logaritmica anche sulla variabile `Area`: 
...
hist(residuals(lm(bcqmed ~ larea, data = tukrivers)))
```

La trasformazione logaritmica ha il vantaggio di permettere di predirre valori della variabile originale usando un modello moltiplicativo facile da implementare e da comunicare agli utenti del modello. Una volta finita la fase di costruzione del modello si potrebbe tornare a valutare la scelta della trasformazione usata per la variabile risposta. 
...
Adesso che abbiamo scelto delle possibili trasformazioni per le variabili esplicative possiamo guardare come queste variano con la variabile dipendente e cercare di usare le variabili trasformate per selezionare un modello che sia adeguato a predirre `lqmed`: 
...
I valori vbassi del Variance Inflation Factor indicano che le variabili non presentano problematicità legate alla co-linearità. 
...
Alcuni punti con un valore di $QMED$ particolarmente piccolo risultano avere una infleunza abbastanza elevata: si potrebbe decidere di eliminare questi punti che sembrano essere anomali ma sarabbe opportuno anche verificare con un esperto di idrologia se queste stazioni hanno caratteristiche particolari. Attenzione: rimouovere questi (pochi) punti dall'analisi implicherebbe che il modello non sarebbe generalizzabile per luoghi le cui caartteristiche sono simili a quelle di queste stazioni. 
...
Le variabili inserite nel modello risultano significative e aumentano la capacità del modello di spiegare la variabilità di `lqmed`: si noti la differenza dei valori di $R^2$ e $R_{Adj}^2$ rispetto al modello `lfit0` e la forte significativà del test F: 
