...
Because of the assumptions made, we are sure the yhat is the function that minimizes the MSE and that no other combination of coefficients can lower that value.
...
The main function to estimate linear models in R is `lm()`

The first variable of `lm` is `formula = target ~ predictor1 + predictor2` where we could have one of more predictors. 
...
1. Call, the model
2. Residuals, the errors of the model which should homoskedastic, have sample variance equal to 0 and constant variance
    + If the distribution was symmetric (which we hope it is), the mean and the median should coincide
    + Moreover, the 1st and 3rd quartile should be specular
3. Coefficients, the beta parameters' values
    + also available through `summary(fit)$coefficients`
4. Residual Standard Error, the estimated variance of the errors $s_{e}$
5. Multiple R-Squared
6. Adjusted R-Squared
7. F-statistic
8. P-value

We can even get the residual degrees of freedom for the model
...
We can see that the estimated value of $\sigma^2$ is present, which is the variance of random variable describing the error of the model, $\varepsilon$: 
...
$\hat{\sigma}$ is used to estimate the uncertainty of the model's coefficient estimates:
